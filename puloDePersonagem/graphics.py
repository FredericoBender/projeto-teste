...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        self.pack()
        master.resizable(1,1) #modified in 2021
        # master.attributes("-fullscreen", True) added in 2021
...
#Buffer by Frederico Bender 
    def ligar_Buffer(self):#Aqui voce pode ativar o Buffer para utilizar uma lista de teclasOBS: usar "checkKey_Buffer()"     
        self.unbind_all("<Key>")
        self.bind_all("<KeyPress>", self._onKeyPress)
        self.bind_all("<KeyRelease>", self._onKeyRelease)
    
    def desligar_Buffer(self): #Se voce quiser pode usar o "checkKey()" padrao desativando as funcoes da lista por aqui
        self.unbind_all("<KeyPress>")
        self.unbind_all("<KeyRelease>")
        self.bind_all("<Key>", self._onKey)
        
    def checkKey_Buffer(self):
    ...
    def _onKeyPress(self, evnt):
        if (evnt.keysym not in self.keys):
